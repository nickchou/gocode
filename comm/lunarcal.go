...
func ToLunarDate(date string) time.Time {
	...
	the_time := time.Date(lunarYear, lm, lunarDay, 0, 0, 0, 0, time.Local)
	return the_time
}

//把日期转换为中国阴历的日期,返回date
...
func calculateLunar(date string) (lunarYear, lunarMonth, lunarDay, leapMonth int, leapMonthFlag bool) {
	...
	myDate, err := time.ParseInLocation(DATE_FORMAT, date, loc)
	if err != nil {
		fmt.Println(err.Error())
	}
	startDate, err := time.ParseInLocation(DATE_FORMAT, START_DATE, loc)
	if err != nil {
		fmt.Println(err.Error())
	}
